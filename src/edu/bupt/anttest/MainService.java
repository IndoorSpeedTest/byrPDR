		...
		public void run() {
			if (d.getState() == Download.STATE_COMPLETE
				...
				if (ave_download_speed > 0 && ave_upload_speed > 0) {
					...
					sb.append("当前测试时间" + date + "\t")
							.append("运营商" + unoTest.networkInfo.getSimOperatorName() + "\t")
							.append("网络类型" + unoTest.networkInfo.getNetworkType() + "\t")
							.append("時延" + ping_latency + "ms" + "\t")
							.append("上行平均"
									+ (float) (Math.round(ave_upload_speed
											* 1024 * 8 / 1048576f * 100)) / 100
									+ "Mbps" + "\t")
							.append("上行最大"
									+ (float) (Math.round(max_upload_speed
											* 1024 * 8 / 1048576f * 100)) / 100
									+ "Mbps" + "\t")
							.append("下行平均"
									+ (float) (Math.round(ave_download_speed
											* 1024 * 8 / 1048576f * 100)) / 100
									+ "Mbps" + "\t")
							.append("下行最大"
									+ (float) (Math.round(max_download_speed
											* 1024 * 8 / 1048576f * 100)) / 100
									+ "Mbps" + "\t").append("lan" + lat + "\t")
							.append("lon" + lon + "\t")
							.append("floor" + floor + "\t");
